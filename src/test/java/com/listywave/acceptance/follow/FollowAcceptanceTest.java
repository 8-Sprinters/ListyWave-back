...
public class FollowAcceptanceTest extends AcceptanceTest {
    ...
    class 팔로우_요청 {
        ...
        void 성공적으로_팔로우_한다() {
            ...
            // when
            ExtractableResponse<Response> response = 팔로우_요청(동호_액세스_토큰, 정수.getId());
        ...
        void 본인을_팔로우_할_수_없다() {
            ...
            // when
            ExtractableResponse<Response> response = 팔로우_요청(동호_액세스_토큰, 동호.getId());
        ...
        void 팔로우가_성공하면_팔로워의_followingCount와_팔로잉의_follwerCount가_1씩_증가한다() {
            ...
            // when
            UserInfoResponse 동호_정보 = 비회원_회원_정보_조회_요청(동호.getId()).as(UserInfoResponse.class);
            UserInfoResponse 정수_정보 = 비회원_회원_정보_조회_요청(정수.getId()).as(UserInfoResponse.class);
    ...
    class 팔로우_취소 {
        ...
        void 팔로우를_성공적으로_취소한다() {
            ...
            // when
            ExtractableResponse<Response> response = 팔로우_취소(동호_액세스_토큰, 정수.getId());
    ...
    class 팔로워_삭제 {
        ...
        void 팔로워를_삭제할_수_있다() {
            ...
            // then
            FollowersResponse 정수_팔로워_목록 = 팔로워_목록_조회(정수.getId()).as(FollowersResponse.class);
    ...
    class 팔로워_목록_조회 {
        ...
        void 성공적으로_팔로워_목록을_조회한다() {
            ...
            // when
            FollowersResponse result = 팔로워_목록_조회(유진.getId()).as(FollowersResponse.class);

            // then
            List<FollowerInfo> expect = List.of(FollowerInfo.of(동호), FollowerInfo.of(정수));
            assertThat(result.followers()).isEqualTo(expect);
        }
        ...
        void 검색어로_팔로워하고_있는_회원을_조회할_수_있다() {
            ...
            // when
            FollowersResponse result = 팔로워_검색(유진.getId(), 동호.getNickname()).as(FollowersResponse.class);

            // then
            List<FollowerInfo> expect = List.of(FollowerInfo.of(동호));
            assertThat(result.followers()).isEqualTo(expect);
        }
    }
    ...
    class 팔로잉_목록_조회 {
        ...
        void 검색어로_팔로잉하고_있는_회원을_조회할_수_있다() {
            ...
            // when
            FollowingsResponse result = 팔로잉_검색(동호.getId(), 유진.getNickname()).as(FollowingsResponse.class);

            // then
            List<FollowingInfo> expect = List.of(FollowingInfo.of(유진));
            assertThat(result.followings()).isEqualTo(expect);
        }
    }
}
